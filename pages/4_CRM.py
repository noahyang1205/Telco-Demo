...
if input_text:
    ...
    if json_response["Database"] == "Operations/Dispatch":
        ...
        if json_response["Request Type"] == "Query":
            prompt = f'Perform the request to the pandas dataframe named dispatch_df. Store results in a variable named results_df. Today is {today}: \n """\n\n\n Request: {original_prompt}, Dataframe columns: {dispatch_columns} """\n'
            pd_responses = openai.Completion.create(
                model="text-davinci-003",
                prompt=prompt,
                temperature=0,
