...
with st.sidebar.expander("See policy data"):
    policy_data = st.text_area("Policy data", policy_data, height=400)

with st.sidebar.expander("See account data"):
    account_data = st.text_area("Account data", account_data, height=400)


today = date.today().strftime("%m/%d/%Y")

bd_chat_lines_context = f'You are a AT&T B2B sales rep support assistant. Your job is to answer the questions using the JSON account database and AT&T policies. Some of the accounts in the database are not current AT&T customers. Today is {today} \n Policy data: """{policy_data}""" \n  JSON account database: """{account_data}"""  '
