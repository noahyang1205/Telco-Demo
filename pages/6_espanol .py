...
st.set_page_config(
    page_title="Telcel chat-bot demo",
    page_icon=":robot:"
)
img = iio.imread('./assets/2560px-Telcel_logo.svg.png')

openai.api_key = os.getenv('OPENAI_API_KEY')

st.image(image=img,width=500)
st.header("Telcel Chat Log")
...
if profile_selection == 'Customized profile':
    ...
    profile_data = Path(os.path.join('./telcel_user_profiles', profile_selection)).read_text("UTF-8")
    expander.write(profile_data)

policy_data = Path(os.path.join('./assets/Telcel_policy.txt')).read_text("UTF-8")
...
if attitude == 'Conservative':
    attitude_prompt = 'Under no circumstances will you offer any credits to the customer. You will tell the customer explicitly that a customer support chatbot cannot offer any credits and Telcel does not have a credit policy'
...
assistant_context = f'You are respectful, welcoming, kind, understanding, professional, truthful, reserved. You are an Telcel customer support AI assistant. Your goal is to answer my questions as accurately as possible and use customer profile data and policy data I provide to help answer them. Do not make up facts or answer questions outside of the scope of customer support. If you are not sure about a question, refer the customer to Telcel website at https://www.telcel.com/ or call the Telcel customer support at 1 (800) 220-9518. Today is {today}\n'
