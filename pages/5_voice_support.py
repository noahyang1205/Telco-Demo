...
def text2voice(text, col):
    ...
    language_code="en-US", name="en-US-Neural2-D",speaking_rate=0.89)
    
    audio_config = texttospeech.AudioConfig(
    audio_encoding=texttospeech.AudioEncoding.LINEAR16,  effects_profile_id=['large-home-entertainment-class-device']
)
...
def input_output_cycle():
    ...
    if audio_bytes:
        ...
        JSON_prompt = f'Based on the request, create a JSON file with the same structure as the example JSON file. """\n\n\n Request: {user_input}, Example JSON: {example_query}. Possible values for Input Type field: 1. User Identification, 2. Add Channel, 3. Remove Channel 4. Channel Information, 5. Greet. Details about each field: 1. User Identification: when user provide first and last name. 2. Add Channel: When user wants to add/subscribe to a new channel. 3. Remove: When user wants to remove/unsubscribe to a channel. 4. Channel Information: When user ask about a channel. 5. Greet: When user greets."""\n\n\n'
