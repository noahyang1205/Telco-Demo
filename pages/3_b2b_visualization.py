...
input_text = st.text_input(
    "Enter Query here: ",
    value="",
    help='Try something like "Get all IPhone and Home devices"',
)
...
foot_traffic = gpd.GeoDataFrame(
    foot_traffic,
    geometry=gpd.points_from_xy(foot_traffic.longitude, foot_traffic.latitude),
)
...
if "m" not in st.session_state:
    st.session_state.m = folium.Map(
        location=[47.55481, -122.67772], tiles="OpenStreetMap", zoom_start=8
    )

if st.session_state.current_response != "" and (
    st.session_state.current_response != st.session_state.past_response
):
    # Only make a query when current response is not empty and not quired already
    print(
        f"current {st.session_state.current_response}, past {st.session_state.past_response}"
    )
    ...
    st.session_state.past_response = (
        st.session_state.current_response
    )  # update past response after query
    ...
    st.session_state.plot_data = [
        [point.xy[1][0], point.xy[0][0]] for point in filtered_df.geometry
    ]
