...
st.set_page_config(
    page_title="Telenor chat-bot demo",
    page_icon=":robot:"
)

openai.api_key = os.getenv('OPENAI_API_KEY')

img = iio.imread('./assets/telenor_logo.png')
st.image(image=img,width=300)
st.header("Telenor Chat Log")
...
policy_data = Path(os.path.join('telenor_policy.txt')).read_text("UTF-8")
...
if attitude == 'Conservative':
    attitude_prompt = 'Under no circumstances will you offer any credits to the customer. You will tell the customer explicitly that a customer support chatbot cannot offer any credits and Telenor does not have a credit policy'
