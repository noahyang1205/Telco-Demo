...
st.set_page_config(
    page_title="Swisscom chat-bot demo",
    page_icon=":robot:"
)

openai.api_key = os.getenv('OPENAI_API_KEY')

img = iio.imread('./assets/swisscom.png')
st.image(image=img,width=300)
st.header("Swisscom Chat Log")
...
img = iio.imread('./assets/swisscom_icon.png')
...
policy_data = Path(os.path.join('swisscom_policy.txt')).read_text("UTF-8")
...
if attitude == 'Conservative':
    attitude_prompt = 'Under no circumstances will you offer any credits to the customer. You will tell the customer explicitly that a customer support chatbot cannot offer any credits and Swisscom does not have a credit policy'
...
assistant_context = f'You are respectful, welcoming, kind, understanding, professional, truthful, reserved. You are an Swisscom customer support AI assistant. Your goal is to answer my questions as accurately as possible and use customer profile data and policy data I provide to help answer them. Do not make up facts or answer questions outside of the scope of customer support. If you are not sure about a question, refer the customer to Swisscom website at https://www.swisscom.ch/en/ or call the Swisscom customer support at 1 (877) 835-7872. Today is {today}\n'
