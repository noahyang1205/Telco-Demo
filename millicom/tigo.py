...
st.set_page_config(page_title="Millicom Tigo sales lead script generator", page_icon=":robot:",layout="wide")
...
product_list = [
    ...
    "Unlimited Elite",
    "Wireless Data 50 GB/month",
    "Wireless Data 100 GB/month",
    "Broadband Core",
    "Broadband Pro",
    "Broadband Ultra",
    "Fiber 300 Mbps",
...
with col1:
    ...
    sales_play = st.selectbox(
        ...
        index=sales_play_list.index(row.iloc[0]["Sales play"]),
    )

    current_product_default = (
        "None"
        if sales_play == "Account Acquisition"
        else list(map(str, row.iloc[0]["Current products"].split(",")))
    )
    # print(f'current products {current_product_default}')
    current_product = st.multiselect(
        "Current products", product_list, current_product_default
    )
    ...
    previous_relationship = st.selectbox(
        ...
        (
            [
                "Personal Connection - Acquaintance",
                "None",
                "Professional",
    ...
    likely_product_needs = st.multiselect(
        "Likely Product Needs", product_list, row.iloc[0]["Product needs"]
    )
...
with col2:
    ...
    email_length = st.slider(
        label="Maximum Email Length", min_value=128, max_value=1024, step=1, value=256
    )
